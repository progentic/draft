...
- A selection reports a canonical value when all effective values match and a
  distinct mixed state when they differ.
...
| :--- | :--- |
| Alignment | `w:jc` |
| Line spacing | `w:spacing` with `w:lineRule="auto"` and an integer line value derived from 240 twips per single line |
...
The conversion is deterministic. For line spacing, the DOCX line value is
`240 * lineSpacingHundredths / 100`; the accepted increment makes the result an
integer. DRAFT must not silently substitute or clamp unsupported values.
...
registry state, related stores, temporary-file promotion state, and the stored
version marker. Lower unsupported and future versions continue to fail
explicitly. No best-effort field repair or guessed value conversion is allowed.
...
- every valid value, boundary, malformed shape, unknown field, and conflicting
  indentation state in mirrored Rust and TypeScript validation;
