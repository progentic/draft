...
Proposed ADR-001 retains the Phase 33 PDF absence scan as a named deferral
...
not imply that the ADR is accepted or that PDF behavior exists; it preserves
the current absence until a governed implementation adds parser-based output,
resource-bound, deterministic-failure, and source-preservation tests.
