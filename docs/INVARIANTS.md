...
| `INV-05` | Accepted | Background jobs persist state per record and resume from the last valid checkpoint after interruption. | `ARCHITECTURE.md` §10 | The absence gate rejects persistent-job implementation before Phase 26. Phase 24 pending PDF candidates start no work and are not jobs. Phase 25 confirms that no job store, scheduler, queue, or product worker exists. Phase 26 must replace the absence gate with persistence and resumability tests before processing begins. | The `verify` job runs the same absence gate; persistence tests become required with the job surface. |
