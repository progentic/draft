...
| `INV-17` | Proposed | Paragraph formatting uses one strict canonical model across editor behavior, Rust validation, persistence, migration, and supported format mappings. Unsupported values and undisclosed lossy saves fail before mutation. | Accepted ADR-004 and `ARCHITECTURE.md` §15.1 | Acceptance-state checks require the binding contract and deny premature controls. Phase 47 must replace absence checks with behavioral validation, migration, persistence, DOCX fidelity, and non-mutation evidence before Phase 48 exposes controls. | The `verify` job enforces accepted decision state, Proposed invariant status, and current product absence. Behavioral enforcement does not exist and no current release gate may treat this invariant as closed. |
...
This invariant remains Proposed. Current enforcement proves that the decision
and contract are accepted while product code has not started implementing the
model. Phase 47 must replace those absence checks with validation,
