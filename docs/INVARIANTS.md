...
| `INV-05` | Accepted | Background jobs persist state per record and resume from the last valid checkpoint after interruption. | `ARCHITECTURE.md` §10 | The Phase 10 absence gate rejects persistent-job implementation before Phase 26. Phase 24 pending PDF candidates start no work and are not jobs. Phase 26 must replace the absence gate with resumability tests before processing begins. | The `verify` job runs the same absence gate; persistence tests become required with the job surface. |
...
| `INV-08` | Accepted | A watched-folder file enters the import pipeline only after stable-write confirmation. | `ARCHITECTURE.md` §10.1 | Phase 24 requires root-confined canonical paths, event-driven deadline resets, a one-second quiet window, unchanged byte length, signature validation, and the chunked-write regression test. It rejects frontend, mutation, persistence, network, and unmanaged-watcher authority. | The `verify` job runs the same Rust tests and import-boundary scans through `scripts/verify.sh`. |
