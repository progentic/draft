...
Phase 47 implements the bounded DOCX source-replacement mechanism under this
existing invariant. The registry retains the canonical source path and imported
fingerprint. A write requires an exact supported-subset disposition or explicit
acceptance of known canonical normalization. Rust rechecks source bytes before
replacement, writes through `write_document_atomically`, commits the new
...
persistence behavior, DOCX property mapping, bounded DOCX import, closed
lossiness handling, and confirmed same-format replacement. It does not yet
