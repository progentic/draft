...
| `INV-02` | Accepted | No Tauri command returns an untyped or generic error. | `ARCHITECTURE.md` §5.1 and §12 | Phases 6 and 9 established typed runtime-status and cancellation commands. The invariant scan rejects generic errors, requires matching command registration and contract-test counts, and compares registered Rust names with frontend wrappers. | The `verify` job runs the same Rust tests, command-contract counts, and Phase 10 bridge-name parity scan. |
