...
| `INV-09` | Accepted | Document saves use atomic write-then-rename. The on-disk file is always the prior complete version or the new complete version, never a partial write. | `ARCHITECTURE.md` §11 | Phase 13 routes every save through `atomic-write-file`, calls `sync_all` before commit, relies on commit for same-directory replacement and parent-directory sync where supported, advances registry state only after a successful write, tests complete create/replace and failed-open behavior, and scans for direct target-write APIs. Phase 14 must add injected interruption, replacement-failure, cleanup, and concurrent-save tests. | The `verify` job runs the same atomic-writer, round-trip, failed-write state, direct-write, command, and frontend boundary checks. Phase 14 hardening remains a required gate. |
...
single-live-handle registry contract, Phase 13 validated load/save and atomic
writer boundaries, ad hoc Rust network clients, and Bash invocation from
...
- Add frontend view integration and close-command coverage when those surfaces
  exist.
- Add Phase 14 interrupted-write, replacement-failure, cleanup, and
  concurrent-save tests before treating atomic save as hardened.
- Add citation, job, product-worker cancellation, and import invariant tests in
  their owning phases.
