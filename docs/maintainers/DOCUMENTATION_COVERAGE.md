...
| AI orchestration | `AiAnalysisRequest`, `assemble_model_request`, `prepare_ai_analysis`, `run_ai_analysis`; stream traits/events | `AI_ORCHESTRATION.md`, `CANCELLATION_BOUNDARY.md`, `CONFIGURATION.md` | `docs/wiki/Current-Limitations.md` | ADR-002 proposed | `INV-07`, `INV-14` | Context, provenance, stream, cancellation, failure tests, and proposal guard | Intentionally internal and not user-facing; proposed ADR-002 would keep provider-backed orchestration outside v1.0.0. |
...
| Text-analysis findings | `TextAnalysisInput`, `TextAnalysisFinding`, five closed finding codes; supporting measurements and Python heuristics | `TEXT_ANALYSIS.md`, `CONFIGURATION.md` | `docs/wiki/Current-Limitations.md` | ADR-002 proposed | `INV-15` | Rust validation, Python determinism, ordering, threshold, offset, limit, authority, provider/model absence, and capability-language guards | Intentionally internal and not user-facing; proposed ADR-002 would make exactly these five heuristics the complete v1 analysis scope after acceptance and Phase 46 implementation. |
