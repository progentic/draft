...
| Text-analysis findings | `TextAnalysisInput`, `TextAnalysisFinding`, five closed finding codes; supporting measurements and Python heuristics | `TEXT_ANALYSIS.md`, `CONFIGURATION.md` | `docs/wiki/Current-Limitations.md` | ADR-002 accepted | `INV-15` | Rust validation, Python determinism, ordering, threshold, offset, limit, authority, provider/model absence, and capability-language guards | Intentionally internal and not user-facing; accepted ADR-002 makes exactly these five heuristics the complete v1 analysis scope after Phase 46 implementation. |
