...
| v1 usability acceptance | Phase 46 visible workflows plus release enforcement in `check-docs.sh`, `check-invariants.sh`, and `check-release-candidate.sh` | `V1_USABILITY_ACCEPTANCE.md`, `PHASE46_WORKFLOWS.md`, `RELEASE_CANDIDATE.md`, `V1_USABILITY_EVIDENCE.md` | Workspace, recovery, and limitation guidance follows the implemented workflows | None; refines the accepted Phase 45 release rule | `INV-UX-01` through `INV-UX-06` | Phase 46 automated interaction evidence plus exact packaged artifacts and manual findings are recorded; a stable complete packaged workflow and later human evidence remain required | Implemented but not release-closed. The ledger tracks `UX-46-001` through `UX-46-024`; exact artifact `75373ffb` closes the native menu, purple identity, compact command, and bottom status defects through `UX-46-023`. Paragraph formatting remains governance-blocked, and every RC and GATE row remains open. |
| Document interoperability and desktop workflows | Phase 47 remains contract-only; Phase 48 adds `NativeMenuItems`, `set_native_menu_state`, `nativeMenu.ts`, `useWorkspaceActions`, Save As, compact command and status bars, and canonical icon assets | ADR-003, accepted `V1_INTEROPERABILITY_AND_DESKTOP_WORKFLOWS.md`, `NATIVE_DESKTOP_WORKFLOW.md`, `RELEASE_CANDIDATE.md` | Workspace guidance describes current File, command-overflow, and status behavior without claiming release closure | ADR-003 Accepted | Existing `INV-03`, `INV-06`, `INV-09`, and `INV-UX-01` through `INV-UX-06`; `INV-UX-07` remains Proposed | Menu-order, shortcut, event, state, dispatcher, Save As, command-overflow keyboard, status-placement, icon-source, package, and release-gate checks plus exact-artifact human review | Exact artifact `75373ffb` passes the bounded Phase 48 packaged checklist and closes `UX-46-021` through `UX-46-023`. This is not final UI-design approval; `RC-08`, `GATE-48`, and governance-blocked paragraph formatting remain open. Phase 47 implementation remains absent. |
