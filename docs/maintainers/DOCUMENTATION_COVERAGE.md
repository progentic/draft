...
repository state through Phase 37. It does not imply that an
...
| Typed Tauri command client | Nine registered Rust commands and matching wrappers under `src/ipc/` | `COMMAND_BOUNDARY.md`, `FRONTEND_COMMAND_CLIENT.md`, `FORMATTING_UX.md`, `OFFLINE_MODE.md` | Formatting review and connectivity mode use validated wrappers | None | `INV-02`, `INV-03`, `INV-10`, `INV-16` | Command serialization tests, wrapper tests, bridge-name parity scan | Implemented and documented; only the existing visible workflows consume wrappers. |
...
| Document open, save, and atomic replacement | `open_document`, `save_document`, `write_document_atomically`; native dialog adapters | `DOCUMENT_SAVE_LOAD.md`, `ERROR_MESSAGES.md` | `docs/wiki/Current-Limitations.md` | None | `INV-03`, `INV-06`, `INV-09` | Command, persistence, interruption, durability, and reopen tests | Intentionally internal and not user-facing; only the backend boundary exists. |
...
| Error presentation | Typed Rust errors, TypeScript guards, runtime/connectivity session state, visible bounded copy | `ERROR_MESSAGES.md`, owning boundary guides | `docs/wiki/Troubleshooting.md` | None | `INV-02`, `INV-03`, `INV-10` | Serialization, exhaustive mapping, fallback, retained-mode, and rendered message tests | Implemented and documented; typed but unwired failures intentionally have no speculative user copy. |
...
Live Wiki publication verified through Phase 36 at commit `5a92643`:
...
The Phase 36 Workspace, Troubleshooting, and Current Limitations updates were
