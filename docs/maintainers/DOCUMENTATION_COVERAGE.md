...
| Document interoperability and desktop workflows | Phase 47 adds the envelope-v2 paragraph model, detached v1 migration, editor preservation, DOCX paragraph mapping, `import_docx_source`, closed `ExternalFidelity` and provenance state, and the path-free `ExternalDocumentSummary`. Same-format DOCX overwrite remains unavailable. Phase 48 adds `NativeMenuItems`, `set_native_menu_state`, `nativeMenu.ts`, `useWorkspaceActions`, Save As, compact command and status bars, and canonical icon assets | ADR-003, ADR-004, accepted interoperability and paragraph contracts, `PARAGRAPH_FORMATTING.md`, `DOCX_INTEROPERABILITY.md`, `NATIVE_DESKTOP_WORKFLOW.md`, `RELEASE_CANDIDATE.md` | Workspace guidance describes bounded DOCX import, source preservation, current File/menu/status behavior, and missing same-format save without claiming paragraph controls or release closure | ADR-003 and ADR-004 Accepted | Existing `INV-03`, `INV-06`, `INV-09`, and `INV-UX-01` through `INV-UX-06`; `INV-17` and `INV-UX-07` remain Proposed | Paragraph bounds/malformed-shape parity, current-only parsing, v1 migration, bounded source growth, canonical path-alias deduplication, ZIP/XML safety, fidelity ordering, provenance fingerprints, no-edit/cancel/failure preservation, path-free DTOs, editor JSON/HTML, DOCX XML, menu, dispatcher, Save As, icon, package, and release-gate checks | DOCX paragraph import is mechanically implemented, but same-format overwrite, parsed Markdown, ODT/RTF disposition, paragraph controls, compatible-reader and packaged human evidence, `UX-46-024`, `RC-07`, `RC-08`, `GATE-47`, and `GATE-48` remain open. Exact artifact `75373ffb` proves only the bounded Phase 48 package checklist, not final UI design. |
