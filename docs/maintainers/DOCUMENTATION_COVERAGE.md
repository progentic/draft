...
| Workspace shell and editor | `DraftWorkspace`, `WorkspaceHeader`, `WorkspaceOperationNotice`, `DocumentOutline`, `DocumentInspector`, `DraftEditor`, `EditorToolbar`, `ConnectivityModeControl` | `WORKSPACE_UI.md`, `OFFLINE_MODE.md`, `PERFORMANCE_MEASUREMENT.md` | `docs/wiki/Workspace.md` | None | `INV-03`, `INV-10`, `INV-UX-03` | `src/App.test.tsx`, operation-notice, connectivity, editor, and benchmark tests | Implemented and documented; replacement-package operation evidence remains pending. |
...
| Document interoperability and desktop workflows | Phase 47 adds the envelope-v2 paragraph model, detached v1 migration, editor preservation, DOCX paragraph mapping, `import_docx_source`, closed `ExternalFidelity` and provenance state, path-free `ExternalDocumentSummary`, and the `save_external_document` source-replacement boundary with one visible workflow. Phase 48 adds `NativeMenuItems`, `set_native_menu_state`, `nativeMenu.ts`, `useWorkspaceActions`, Save As, compact command and status bars, and canonical icon assets | ADR-003, ADR-004, accepted interoperability and paragraph contracts, `PARAGRAPH_FORMATTING.md`, `DOCX_INTEROPERABILITY.md`, `NATIVE_DESKTOP_WORKFLOW.md`, `RELEASE_CANDIDATE.md` | Workspace guidance distinguishes `.draft` Save, DOCX export, and confirmed Save Back without claiming paragraph controls or release closure | ADR-003 and ADR-004 Accepted | Existing `INV-03`, `INV-06`, `INV-09`, and `INV-UX-01` through `INV-UX-06`; `INV-17` and `INV-UX-07` remain Proposed | Paragraph bounds/malformed-shape parity, current-only parsing, v1 migration, bounded source growth, canonical path-alias deduplication, ZIP/XML safety, part-relative relationship containment, fidelity ordering, provenance fingerprints, real-import stale-source denial, named normalization consent, non-mutating eligibility, atomic replacement and rollback, path-free typed recovery, a hashed Word-authored import/export fixture, LibreOffice text comparison, exact rendered Open-response and stale-feedback regressions, editor JSON/HTML, DOCX XML, menu, dispatcher, Save As, Save Back, icon, package, and release-gate checks | Artifact `8e974736` visibly ran commit `14363903` and passed DOCX export, closing only `UX-47-011`; DOCX Open still failed. The exact Word response and stale-feedback correction are mechanically green, but replacement packaged Open evidence is absent. Parsed Markdown, ODT/RTF disposition, paragraph controls, `UX-46-024`, `RC-07`, `RC-08`, `GATE-47`, and `GATE-48` remain open. Exact artifact `75373ffb` proves only the bounded Phase 48 package checklist, not final UI design. |
