...
| Packaging and application icons | `assets/DRAFT_Logo.png`; `package:macos`; `package-macos.sh`; `check-packaging.sh`; active `app` target; five explicit icon paths | `PACKAGING.md`, `CONFIGURATION.md`, `TOOLCHAIN.md`, `NATIVE_DESKTOP_WORKFLOW.md` | `docs/wiki/Current-Limitations.md` | None | `INV-13` for verification parity | Canonical-source and derivative hashes, structured config checks, clean unsigned Apple Silicon build, plist, architecture, executable, embedded-icon validation, and visible header test | Mechanically implemented from one canonical purple source; Finder, Dock, switcher, and packaged-window identity remain open Phase 48 manual evidence. |
