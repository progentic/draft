...
| Document interoperability and desktop workflows | Phase 47 remains contract-only; Phase 48 adds `NativeMenuItems`, `set_native_menu_state`, `nativeMenu.ts`, `useWorkspaceActions`, Save As, and canonical icon assets | ADR-003, accepted `V1_INTEROPERABILITY_AND_DESKTOP_WORKFLOWS.md`, `NATIVE_DESKTOP_WORKFLOW.md`, `RELEASE_CANDIDATE.md` | Workspace and limitations guidance describes current File actions without claiming release closure | ADR-003 Accepted | Existing `INV-03`, `INV-06`, `INV-09`, and `INV-UX-01` through `INV-UX-06`; `INV-UX-07` remains Proposed | Menu-order, shortcut, event, state, dispatcher, Save As, icon-source, package, and release-gate checks | Phase 48 is mechanically implemented for the bounded File/menu/icon scope; packaged manual evidence is pending and `RC-08`/`GATE-48` remain open. Phase 47 implementation remains absent. |
