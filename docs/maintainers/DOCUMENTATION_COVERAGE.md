...
| Document interoperability and desktop workflows | Phase 47 adds the envelope-v2 paragraph model, detached v1 migration, editor preservation, DOCX paragraph/run/page-break mapping, explicit page surfaces, `import_docx_source`, closed `ExternalFidelity` and provenance state, path-free `ExternalDocumentSummary`, and the `save_external_document` source-replacement boundary with one visible workflow. Phase 48 adds `NativeMenuItems`, `set_native_menu_state`, `set_window_title`, `nativeMenu.ts`, `useWorkspaceActions`, Save As, Rust-owned filename suggestions, compact command and status bars, About build identity, and canonical icon assets | ADR-003, ADR-004, accepted interoperability and paragraph contracts, `PARAGRAPH_FORMATTING.md`, `DOCX_INTEROPERABILITY.md`, `NATIVE_DESKTOP_WORKFLOW.md`, `RELEASE_CANDIDATE.md` | Workspace guidance distinguishes `.draft` Save, DOCX export, and confirmed Save Back without claiming paragraph controls or release closure | ADR-003 and ADR-004 Accepted | Existing `INV-03`, `INV-06`, `INV-09`, and `INV-UX-01` through `INV-UX-06`; `INV-17` and `INV-UX-07` remain Proposed | Paragraph and direct run bounds, current-only parsing, v1 migration, canonical page-break JSON/HTML/DOCX round trip and visual separation, bounded source growth, path-alias deduplication, ZIP/XML safety, relationship containment, fidelity ordering, provenance fingerprints, stale-source denial, normalization consent, atomic rollback, path-free recovery, title/build identity, filename suggestions, a hashed Word fixture, LibreOffice text comparison, rendered formatting/Open regressions, menu, dispatcher, Save As, Save Back, icon, package, and release-gate checks | Artifacts `8e974736` and `2dfe312b` separately pass export and basic Open. Artifact `91fe1ba9` retained substantially more formatting but exposed page-surface, title/build-placement, and Save-suggestion defects. Parsed Markdown, ODT/RTF disposition, paragraph controls, `UX-46-024`, `UX-47-013` through `UX-47-019`, `RC-07`, `RC-08`, `GATE-47`, and `GATE-48` remain open. Exact artifact `75373ffb` proves only the bounded Phase 48 package checklist, not final UI design. |
