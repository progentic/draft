...
| v1 usability acceptance | No new product surface; proposal enforcement in `check-docs.sh`, `check-invariants.sh`, and `check-release-candidate.sh` | `V1_USABILITY_ACCEPTANCE.md`, `RELEASE_CANDIDATE.md`; future cumulative evidence in `V1_USABILITY_EVIDENCE.md` | Future first-run, Start Here, shortcut, and recovery guidance must follow implemented workflows | None; proposes a refinement of the accepted Phase 45 release rule | Proposed `INV-UX-01` through `INV-UX-06` | Structural proposal checks now; Phase 46-49 automated, packaged, and uncoached human evidence remains absent | Proposed and blocked by contract review. Open `RC-*` and `GATE-*` rows continue to block release; no workflow is claimed. |
