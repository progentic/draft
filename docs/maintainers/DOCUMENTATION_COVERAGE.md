...
| Release-candidate hardening | `check-release-candidate.sh`; blocker and gate inventory | `RELEASE_CANDIDATE.md`, `PACKAGING.md`, `CONFIGURATION.md`, `TOOLCHAIN.md` | Existing canonical limitations; no user behavior changed | ADR-001 for accepted PDF deferral | Existing invariants remain authoritative | Structured inventory counts, live CSP/package/workflow evidence, pre-release version/tag state, generated-artifact scan, and full verifier | Implemented as a Phase 44 hardening baseline; six release blockers and four mandatory pre-49 gates remain open, so this is not final-candidate readiness. |
