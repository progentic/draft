...
| Error presentation | `errorPresentation.ts`; runtime/connectivity session state; formatting panel; citation node view | `ERROR_UX.md`, `ERROR_MESSAGES.md`, owning boundary guides | `docs/wiki/Troubleshooting.md`, `docs/wiki/Workspace.md` | None | `INV-02`, `INV-03`, `INV-10` | Exhaustive code/disposition, fallback, retained-state, announcement, focus, and control-semantics tests | Implemented and documented for four already-visible surfaces; typed but unwired failures intentionally have no speculative user copy. |
