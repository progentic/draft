...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| Desktop runtime and managed state | `src-tauri/src/lib.rs`; `application::{network_client,reference_store,job_store,secret_store,runtime_status,open_requests}`; `run` | `TOOLCHAIN.md`, `COMMAND_BOUNDARY.md`, `CONFIGURATION.md`, `SECRET_STORAGE.md`, `PACKAGING.md` | `docs/wiki/Workspace.md`, `docs/wiki/Current-Limitations.md` | Baseline stack decision | `INV-01`, `INV-03`, `INV-10`, `INV-13` | Rust application, command, store, package-identity, application-open queue, and parity tests | Implemented and documented; replacement-package human evidence remains pending. |
| Workspace shell and editor | `DraftWorkspace`, `WorkspaceHeader`, `WorkspaceOperationNotice`, `WorkspaceStatusBar`, `useWindowTitle`, `DocumentOutline`, `DocumentInspector`, `DraftEditor`, `EditorToolbar`, `ConnectivityModeControl` | `WORKSPACE_UI.md`, `NATIVE_DESKTOP_WORKFLOW.md`, `OFFLINE_MODE.md`, `PERFORMANCE_MEASUREMENT.md` | `docs/wiki/Workspace.md` | None | `INV-03`, `INV-10`, `INV-UX-03` | `src/App.test.tsx`, window-title, page-break presentation, operation-notice, connectivity, editor, and benchmark tests | Implemented and documented; native title, build placement, explicit page surfaces, and Save suggestions require replacement-package evidence. |
| Runtime status and visible failures | `get_runtime_status`; `draft://runtime-status`; `draft://application-open`; `draft://native-menu-action`; `startRuntimeStatusSession`; `listenToApplicationOpenRequests`; `takeApplicationOpenRequest`; `listenToNativeMenuActions`; `useRuntimeStatus`; `runtimeFailurePresentation` | `COMMAND_BOUNDARY.md`, `EVENT_BOUNDARY.md`, `FRONTEND_COMMAND_CLIENT.md`, `ERROR_MESSAGES.md`, `ERROR_UX.md` | `docs/wiki/Troubleshooting.md` | None | `INV-02`, `INV-03`, `INV-UX-03`, `INV-UX-04` | Rust command/event tests, strict frontend validators, operation-notice interactions, and exhaustive runtime presentation suites | Implemented and documented; exact replacement-package evidence remains pending. |
| Typed Tauri command client | Nineteen registered Rust commands and matching wrappers under `src/ipc/` | `COMMAND_BOUNDARY.md`, `FRONTEND_COMMAND_CLIENT.md`, `FORMATTING_UX.md`, `OFFLINE_MODE.md`, `AUDIT_DIAGNOSTICS.md`, `PHASE46_WORKFLOWS.md`, `NATIVE_DESKTOP_WORKFLOW.md`, `DOCX_INTEROPERABILITY.md` | Document, reference/citation, text-check, formatting, connectivity, DOCX, native File menu, and macOS document-open workflows use validated wrappers | None | `INV-01`, `INV-02`, `INV-03`, `INV-10`, `INV-16` | Command serialization tests, wrapper tests, bridge-name parity scan | Implemented and documented; path-free application open and external DOCX writing remain Rust-owned. |
...
| Document create, open, import, save, close, and atomic replacement | `create_document`, `open_document`, `open_application_document`, `save_document`, `SaveAsFormat`, `compile_plain_text`, `close_document`, `text_import`, `write_document_atomically`, `useDocumentSession`; async native dialog and macOS run-event adapters | `DOCUMENT_SAVE_LOAD.md`, `PHASE46_WORKFLOWS.md`, `ERROR_MESSAGES.md`, `CRITICAL_PATHS.md`, `PACKAGING.md` | `docs/wiki/Workspace.md`, `docs/wiki/Troubleshooting.md` | None | `INV-03`, `INV-06`, `INV-09`, `INV-UX-03` | Rust-owned identity, explicit origin, bounded import, source preservation, path-free application open, three-format Save As, deterministic text conversion, visible outcomes, persistence, cancellation non-mutation, interruption, durability, reopen, dirty-state, handle-release, and Phase 41 critical-path tests | Implemented and visible in the branch; only DRAFT Save As rebinds authority. DOCX/plain-text outputs preserve title and dirty state. Replacement-package evidence remains pending. |
...
| Data migration baseline | Version 2 document envelope with named v1 migration; version 1 citation attrs, reference payload, and reference-store dispatch | `DATA_MIGRATION.md`, `DOCUMENT_ENVELOPE.md`, `PARAGRAPH_FORMATTING.md`, `CITATION_NODE.md`, `REFERENCE_RECORD.md`, `REFERENCE_STORE.md`, `CONFIGURATION.md` | None; no visible migration workflow exists | ADR-004 | `INV-04`, `INV-09`; `INV-17` Proposed | v1-to-v2 migration, first-save promotion, lower/future rejection, source/registry/row non-mutation, transactional store initialization and rollback | Implemented and documented for one explicit document transition; reference and citation schemas retain fail-closed version 1 behavior. |
...
| DOCX export | `compile_docx`, `export_docx`, `save_document`, `saveDocumentAs`; strict model/package compiler and atomic write behind the unified Save As boundary | `DOCX_EXPORT.md`, `PHASE46_WORKFLOWS.md`, `CONFIGURATION.md`, `ERROR_MESSAGES.md`, `CRITICAL_PATHS.md` | `docs/wiki/Workspace.md`, `docs/wiki/Troubleshooting.md`, `docs/wiki/Current-Limitations.md` | None | `INV-04`, `INV-09` | Parser, package, XML, font run-property, mixed-format, resource-limit, citation-rejection, atomic-export, source-preservation, IPC, and interaction tests | Implemented through typed Save As as a non-authoritative copy; PDF and unsupported DOCX content remain unavailable, and packaged selector evidence remains open. |
...
| Packaging and application icons | `assets/DRAFT_Logo.png`; `package:macos`; `package-macos.sh`; `check-packaging.sh`; active `app` target; five explicit icon paths | `PACKAGING.md`, `CONFIGURATION.md`, `TOOLCHAIN.md`, `NATIVE_DESKTOP_WORKFLOW.md` | `docs/wiki/Current-Limitations.md` | None | `INV-13` for verification parity | Canonical-source and derivative hashes, structured config checks, clean unsigned Apple Silicon build, plist, architecture, executable, embedded-icon validation, visible header test, and exact-artifact review | Exact artifact `75373ffb` passed Finder, Dock, switcher, and packaged-window identity review. Signing, notarization, final distribution, and final UI-design approval remain absent. |
...
| v1 usability acceptance | Phase 46 visible workflows plus release enforcement in `check-docs.sh`, `check-invariants.sh`, and `check-release-candidate.sh` | `V1_USABILITY_ACCEPTANCE.md`, `PHASE46_WORKFLOWS.md`, `RELEASE_CANDIDATE.md`, `V1_USABILITY_EVIDENCE.md` | Workspace, recovery, and limitation guidance follows the implemented workflows | ADR-004 accepts the paragraph direction without closing usability evidence | `INV-UX-01` through `INV-UX-06`; `INV-17` remains Proposed | Phase 46 automated interaction evidence plus exact packaged artifacts and manual findings are recorded; a stable complete packaged workflow and later human evidence remain required | Implemented but not release-closed. The ledger tracks `UX-46-001` through `UX-46-024`; exact artifact `75373ffb` closes the native menu, purple identity, compact command, and bottom status defects through `UX-46-023`. Paragraph controls and evidence remain pending, and every RC and GATE row remains open. |
| Document interoperability and desktop workflows | Phase 47 adds the envelope-v2 paragraph model, detached v1 migration, editor preservation, DOCX paragraph/run/page-break mapping, explicit page surfaces, a detailed DOCX parser plus bounded readable fallback, lossy table/footnote/list conversion, `import_docx_source`, closed `ExternalFidelity` and provenance state, path-free `ExternalDocumentSummary`, and the `save_external_document` source-replacement boundary with one visible workflow. Phase 48 adds `NativeMenuItems`, `set_native_menu_state`, `set_window_title`, `nativeMenu.ts`, `useWorkspaceActions`, Save As, Rust-owned filename suggestions, compact command and status bars, About build identity, and canonical icon assets | ADR-003, ADR-004, accepted interoperability and paragraph contracts, `PARAGRAPH_FORMATTING.md`, `DOCX_INTEROPERABILITY.md`, `NATIVE_DESKTOP_WORKFLOW.md`, `RELEASE_CANDIDATE.md` | Workspace guidance distinguishes `.draft` Save, DOCX export, confirmed Save Back, and source-preserving readable DOCX copies without claiming paragraph controls or native table/footnote editing | ADR-003 and ADR-004 Accepted | Existing `INV-03`, `INV-06`, `INV-09`, and `INV-UX-01` through `INV-UX-06`; `INV-17` and `INV-UX-07` remain Proposed | Paragraph and direct run bounds, current-only parsing, v1 migration, canonical page-break JSON/HTML/DOCX round trip and visual separation, disclosed paragraph normalization, unfamiliar-wrapper and drawing-text fallback, no-readable-content denial, lossy table rows and referenced end notes, bounded source growth, path-alias deduplication, ZIP/XML safety, relationship containment, fidelity ordering, provenance fingerprints, stale-source denial, normalization consent, atomic rollback, path-free recovery, title/build identity, filename suggestions, a hashed Word fixture, LibreOffice text comparison, rendered formatting/Open regressions, menu, dispatcher, Save As, Save Back, icon, package, and release-gate checks | Artifacts `8e974736` and `2dfe312b` separately pass export and basic Open. Artifact `91fe1ba9` retained substantially more formatting but exposed page-surface, title/build-placement, and Save-suggestion defects. The current two-tier parser passes focused tests and a temporary 103-path read-only local corpus probe but has no replacement package evidence. Parsed Markdown, ODT/RTF disposition, paragraph controls, `UX-46-024`, `UX-47-010`, `UX-47-013` through `UX-47-019`, `RC-07`, `RC-08`, `GATE-47`, and `GATE-48` remain open. Exact artifact `75373ffb` proves only the bounded Phase 48 package checklist, not final UI design. |
