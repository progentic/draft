...
| RC-08 | Release blocker | Open | Native File actions, shared dispatch, Save As, and canonical purple icon assets are mechanically implemented, but packaged menu, shortcut, state, Finder, Dock, switcher, branding, and desktop-layout evidence is incomplete; findings `UX-46-016`, `UX-46-017`, `UX-46-020`, and `UX-46-021` remain open pending retest. | Desktop UI and native workflow | Phase 48 | Native menus, visible controls, and shortcuts share one state-aware action path and pass packaged icon, layout, focus, enablement, and keyboard evidence. |
