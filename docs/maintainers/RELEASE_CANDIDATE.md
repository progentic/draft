...
| RC-07 | Release blocker | Open | Phase 47 mechanically implements bounded DOCX import, fidelity/lossiness state, provenance, fingerprint validation, source replacement, rollback, and path-free recovery. Artifact `8e974736` visibly ran commit `14363903` and passed DOCX export, closing only `UX-47-011`; packaged DOCX Open still failed because settled export feedback masked the later session outcome. The exact Word response now passes the rendered Open-action regression, but a replacement packaged-human Open run is absent. Parsed Markdown, governed ODT/RTF dispositions, and complete format evidence also remain absent. | Document interoperability | Phase 47 | Supported-format fixtures, Rust-owned lifecycle state, explicit fidelity classes, no-edit source preservation, typed failures, and lossless or rejected-lossy save behavior pass. |
