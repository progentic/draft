...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| RC-01 | Release blocker | Open | New, Open, Save, Close, dirty-state protection, and registry-handle release are implemented and pass browser interaction tests; a stable complete packaged-app lifecycle run is still missing. | Document workflow | Phase 46 | The visible workflow uses existing Rust-owned lifecycle paths, protects dirty state, passes browser and packaged-app tests, and matches user documentation. |
...
| GATE-46 | Must close before Phase 49 | Open | Automated accessibility, keyboard, focus, announcement, state, recovery, responsive-layout, and visible-workflow checks pass; the stable complete packaged workflow required to close `RC-01` through `RC-04` remains missing. | Frontend accessibility and workflow clarity | Phase 46 | Critical flows satisfy the accepted discoverability, understanding, state, recovery, keyboard, focus, naming, announcement, terminology, and unavailable-state criteria. |
