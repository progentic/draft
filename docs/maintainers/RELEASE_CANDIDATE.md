...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| RC-01 | Release blocker | Open | Exact-artifact validation confirms New, Open, Save, Close, and effective font-control state work, closing their specific historical findings; a stable complete packaged-app lifecycle run is still missing. | Document workflow | Phase 46 | The visible workflow uses existing Rust-owned lifecycle paths, protects dirty state, passes browser and packaged-app tests, and matches user documentation. |
...
| RC-04 | Release blocker | Open | The Rust-owned DOCX compiler is reachable through the typed Save As selector with source-preservation tests; a stable packaged success/failure/recovery run is still missing. | Export workflow | Phase 46 | The supported DOCX path is reachable through a clear Rust-owned target flow and passes packaged source-preservation and recovery tests. |
...
| RC-07 | Release blocker | Open | Phase 47 mechanically implements bounded DOCX import, fidelity/lossiness state, provenance, fingerprint validation, source replacement, rollback, path-free recovery, and a closed DRAFT/DOCX/plain-text Save As selector. The detailed parser preserves supported formatting; tables, notes, bounded paragraph approximations, and unfamiliar safe Word wrappers can enter a disclosed readable lossy copy while source replacement remains denied. Focused tests and a temporary 103-path local corpus probe pass, but replacement-package validation has not run. Title transitions, parsed Markdown, governed ODT/RTF dispositions, compatible-reader evidence, and complete format evidence remain open. | Document interoperability | Phase 47 | Supported-format fixtures, Rust-owned lifecycle state, explicit fidelity classes, no-edit source preservation, typed failures, disclosed lossy import with denied lossy save-back, and safe supported-subset output pass. |
| RC-08 | Release blocker | Open | Exact artifact `75373ffb` passed packaged review of the native File menu, compact controls, shared dispatch, busy and disabled states, Save As, bottom status placement, keyboard behavior, narrow layout, and purple identity, closing `UX-46-021` through `UX-46-023`. This is not final UI-design approval. ADR-004 is accepted and the model foundation exists, but `UX-46-024` controls and evidence remain open. | Desktop UI and native workflow | Phase 48 | Native menus, visible controls, and shortcuts share one state-aware action path and pass packaged icon, layout, focus, enablement, and keyboard evidence; paragraph controls require the complete accepted model behavior and manual proof. |
...
| GATE-46 | Roadmap gate | Open | Exact-artifact validation closes the specific New, Save, and font-control defects, but the complete eight-step workflow did not pass and all RC rows remain open. Accepted ADR-003 assigns confirmed native-menu and packaged-icon findings `UX-46-020` and `UX-46-021` to Phase 48 without closing this gate. | Frontend accessibility and workflow clarity | Phase 46 | `RC-01` through `RC-04` close and critical flows satisfy accepted discoverability, understanding, state, recovery, keyboard, focus, naming, announcement, terminology, and unavailable-state criteria. |
| GATE-47 | Roadmap gate | Open | The envelope-v2 model, migration, DOCX mapping, bounded detailed and readable parsers, external-source lifecycle, fidelity state, visible source replacement, and three-format Save As boundary exist mechanically. Common Word metadata preserves visible text; tables, notes, and unfamiliar safe structures can open as source-preserving lossy copies. No replacement package has passed this correction; title, parsed Markdown, ODT/RTF disposition, compatible-reader, and complete format evidence remain open. | Document interoperability | Phase 47 | `RC-07` closes with the accepted format matrix, lifecycle, fidelity, lossiness, source-preservation, and fixture evidence. |
| GATE-48 | Roadmap gate | Open | Exact artifact `75373ffb` passed the bounded Phase 48 packaged-human checklist and closed `UX-46-021` through `UX-46-023`. The result validates this implementation, not a final UI design; `RC-08` and `UX-46-024` remain open pending paragraph controls and evidence. | Desktop UI and native workflow | Phase 48 | `RC-08` closes with shared dispatch, native-menu parity, state, icon, layout, focus, and keyboard evidence. |
