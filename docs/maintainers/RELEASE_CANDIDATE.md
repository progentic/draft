...
| RC-07 | Release blocker | Open | Phase 47 mechanically implements bounded DOCX import, fidelity/lossiness state, provenance, fingerprint validation, source replacement, rollback, path-free recovery, and a closed DRAFT/DOCX/plain-text Save As selector. Table cells, referenced footnotes, and unrepresented list numbering now enter a disclosed readable lossy copy while source replacement remains denied; focused parser and frontend tests pass, but replacement-package validation has not run. Title transitions, parsed Markdown, governed ODT/RTF dispositions, compatible-reader evidence, and complete format evidence remain open. | Document interoperability | Phase 47 | Supported-format fixtures, Rust-owned lifecycle state, explicit fidelity classes, no-edit source preservation, typed failures, disclosed lossy import with denied lossy save-back, and safe supported-subset output pass. |
...
| GATE-47 | Roadmap gate | Open | The envelope-v2 model, migration, DOCX mapping, bounded reader, external-source lifecycle, fidelity state, visible source replacement, and three-format Save As boundary exist mechanically. Common Word metadata preserves visible text, while table cells and referenced footnotes now import through a source-preserving readable approximation. No replacement package has passed this correction; title, parsed Markdown, ODT/RTF disposition, compatible-reader, and complete format evidence remain open. | Document interoperability | Phase 47 | `RC-07` closes with the accepted format matrix, lifecycle, fidelity, lossiness, source-preservation, and fixture evidence. |
