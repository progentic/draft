...
| RC-07 | Release blocker | Open | Phase 47 mechanically implements bounded DOCX paragraph import, closed fidelity/lossiness categories, Rust-owned provenance, source-fingerprint validation, typed non-mutating eligibility, exact or consented-normalization replacement, atomic rollback, path-free recovery, and one visible confirmed Save Back workflow. Local macOS reader-open evidence covers exact and accepted-normalized output. Parsed Markdown, governed ODT/RTF dispositions, broader compatible-reader comparison, packaged human validation, and complete format evidence remain absent. | Document interoperability | Phase 47 | Supported-format fixtures, Rust-owned lifecycle state, explicit fidelity classes, no-edit source preservation, typed failures, and lossless or rejected-lossy save behavior pass. |
...
| GATE-47 | Roadmap gate | Open | The envelope-v2 paragraph model, migration, editor preservation, DOCX property mapping, bounded DOCX reader, external-source lifecycle, fidelity/lossiness decisions, confirmed visible source replacement, and local reader-open evidence exist mechanically. Parsed Markdown, ODT/RTF disposition, packaged save evidence, broader compatible-reader comparison, and complete evidence do not. | Document interoperability | Phase 47 | `RC-07` closes with the accepted format matrix, lifecycle, fidelity, lossiness, source-preservation, and fixture evidence. |
