...
| RC-07 | Release blocker | Open | Phase 47 mechanically implements bounded DOCX import, fidelity/lossiness state, provenance, fingerprint validation, source replacement, rollback, path-free recovery, and a closed DRAFT/DOCX/plain-text Save As selector. The broader four-file audit found one successful import, two table-dependent packages, and one footnote-dependent package after common Word metadata handling. Tables and footnotes remain intentionally unsupported until DRAFT has preservable canonical models. Title transitions, Save suggestions, parsed Markdown, governed ODT/RTF dispositions, compatible-reader evidence, and complete format evidence remain open. | Document interoperability | Phase 47 | Supported-format fixtures, Rust-owned lifecycle state, explicit fidelity classes, no-edit source preservation, typed failures, and lossless or rejected-lossy save behavior pass. |
...
| GATE-47 | Roadmap gate | Open | The envelope-v2 model, migration, DOCX mapping, bounded reader, external-source lifecycle, fidelity state, visible source replacement, and three-format Save As boundary exist mechanically. Common Word metadata now preserves visible text, but the broader source audit still exposes missing table and footnote models and no replacement package has passed. Title, suggestion, parsed Markdown, ODT/RTF disposition, compatible-reader, and complete format evidence remain open. | Document interoperability | Phase 47 | `RC-07` closes with the accepted format matrix, lifecycle, fidelity, lossiness, source-preservation, and fixture evidence. |
