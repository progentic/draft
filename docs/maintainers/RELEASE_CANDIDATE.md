...
| RC-07 | Release blocker | Open | Phase 47 mechanically implements bounded DOCX paragraph import, closed fidelity/lossiness categories, Rust-owned provenance, source-fingerprint validation, typed non-mutating eligibility, exact or consented-normalization replacement, atomic rollback, path-free recovery, and one visible confirmed Save Back workflow. Artifact `910a204f` failed normalized consent, stale-source presentation, entry-point parity, and complete compatible-reader evidence. Replacement candidate `cb363385` names normalizations, proves the real imported-source fingerprint path, and passes local text extraction plus configured LibreOffice conversion, but no replacement package has passed human review. Parsed Markdown, governed ODT/RTF dispositions, and complete format evidence remain absent. | Document interoperability | Phase 47 | Supported-format fixtures, Rust-owned lifecycle state, explicit fidelity classes, no-edit source preservation, typed failures, and lossless or rejected-lossy save behavior pass. |
...
| GATE-47 | Roadmap gate | Open | The envelope-v2 paragraph model, migration, editor preservation, DOCX property mapping, bounded DOCX reader, external-source lifecycle, fidelity/lossiness decisions, visible source replacement, and local reader evidence exist mechanically. Artifact `910a204f` failed its human gate; its correction requires a new package and complete retest. Parsed Markdown, ODT/RTF disposition, packaged save evidence, and complete compatible-reader evidence do not exist. | Document interoperability | Phase 47 | `RC-07` closes with the accepted format matrix, lifecycle, fidelity, lossiness, source-preservation, and fixture evidence. |
