...
| RC-03 | Release blocker | Open | Proposed ADR-002 is under review. Deterministic text-analysis findings have no start command, packaged helper discovery, or visible review workflow; provider-backed analysis remains internal and outside the proposed v1 boundary. | Analysis workflow | Phase 46 | After the ADR-002 governance gate closes, the five named local checks execute deterministically without network or credentials, expose typed recovery and accessible review behavior, pass boundary and packaged interaction tests, and make no generative or semantic claim. |
