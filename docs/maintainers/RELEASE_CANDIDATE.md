...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| RC-01 | Release blocker | Open | Exact-artifact validation confirms New, Open, and Save now work, closing their specific historical findings. Font-control state finding `UX-46-019` remains open, and a stable complete packaged-app lifecycle run is still missing. | Document workflow | Phase 46 | The visible workflow uses existing Rust-owned lifecycle paths, protects dirty state, passes browser and packaged-app tests, and matches user documentation. |
...
| RC-08 | Release blocker | Open | Primary workflows lack native macOS menu integration, shared action dispatch, final window branding, and accepted desktop-layout evidence; manual findings `UX-46-016`, `UX-46-017`, `UX-46-020`, and `UX-46-021` confirm the menu, grouping, and icon failures. | Desktop UI and native workflow | Phase 48 | Native menus, visible controls, and shortcuts share one state-aware action path and pass packaged icon, layout, focus, enablement, and keyboard evidence. |
...
| GATE-46 | Roadmap gate | Open | Exact-artifact validation closes the specific New and Save defects, but font-control finding `UX-46-019`, the remaining Phase 46 workflow evidence, and all RC rows remain open. Accepted ADR-003 assigns interoperability and desktop findings, including `UX-46-020` and `UX-46-021`, to later phases without closing this gate. | Frontend accessibility and workflow clarity | Phase 46 | `RC-01` through `RC-04` close and critical flows satisfy accepted discoverability, understanding, state, recovery, keyboard, focus, naming, announcement, terminology, and unavailable-state criteria. |
