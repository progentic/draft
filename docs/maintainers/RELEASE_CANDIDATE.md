...
| RC-07 | Release blocker | Open | Phase 47 mechanically implements bounded DOCX import, fidelity/lossiness state, provenance, fingerprint validation, source replacement, rollback, and path-free recovery. Artifact `8e974736` passed DOCX export. Artifact `2dfe312b` passed basic DOCX Open but failed source-format fidelity. Candidate `91fe1ba9` retained substantially more supported formatting, but its packaged review still found continuous-page presentation, title/build placement, and Save-suggestion defects. Parsed Markdown, governed ODT/RTF dispositions, compatible-reader evidence, and complete format evidence also remain absent. | Document interoperability | Phase 47 | Supported-format fixtures, Rust-owned lifecycle state, explicit fidelity classes, no-edit source preservation, typed failures, and lossless or rejected-lossy save behavior pass. |
...
| GATE-47 | Roadmap gate | Open | The envelope-v2 model, migration, DOCX mapping, bounded reader, external-source lifecycle, fidelity state, and visible source replacement exist mechanically. Packaged export and basic Open pass separately. Candidate `91fe1ba9` preserved more accepted formatting but failed explicit page-surface, title/build-placement, and Save-suggestion review. Parsed Markdown, ODT/RTF disposition, packaged correction evidence, and complete format evidence remain open. | Document interoperability | Phase 47 | `RC-07` closes with the accepted format matrix, lifecycle, fidelity, lossiness, source-preservation, and fixture evidence. |
