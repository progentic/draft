...
| RC-08 | Release blocker | Open | Manual review closed the native File-menu hierarchy finding. Shared dispatch, Save As, compact command and status bars, and canonical purple icon assets are mechanically implemented, but packaged parity, busy-state, Finder, Dock, switcher, branding, layout, focus, and keyboard evidence is incomplete; findings `UX-46-016`, `UX-46-017`, and `UX-46-021` through `UX-46-024` remain open. Paragraph formatting is governance-blocked and has no product implementation. | Desktop UI and native workflow | Phase 48 | Native menus, visible controls, and shortcuts share one state-aware action path and pass packaged icon, layout, focus, enablement, and keyboard evidence; any expanded paragraph model first completes its separate governance path. |
...
| GATE-48 | Roadmap gate | Open | The bounded File menu, shared action dispatcher, Save As path, compact command and status bars, and icon chain are implemented with automated evidence; complete packaged and desktop-layout validation does not yet exist. | Desktop UI and native workflow | Phase 48 | `RC-08` closes with shared dispatch, native-menu parity, state, icon, layout, focus, and keyboard evidence. |
