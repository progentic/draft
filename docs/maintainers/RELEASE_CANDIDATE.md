...
| RC-07 | Release blocker | Open | Phase 47 mechanically implements bounded DOCX import, fidelity/lossiness state, provenance, fingerprint validation, source replacement, rollback, and path-free recovery. Artifact `c3b2b54c` proved its `7ec149de` release identity but failed the primary DOCX workflow: a normal Word package was rejected and Export produced no usable result. The correction identifies a valid part-relative relationship target as the import cause and adds an independent Word-to-DRAFT-to-DOCX round trip with LibreOffice text comparison; a corrected packaged-human run is still absent. Parsed Markdown, governed ODT/RTF dispositions, and complete format evidence also remain absent. | Document interoperability | Phase 47 | Supported-format fixtures, Rust-owned lifecycle state, explicit fidelity classes, no-edit source preservation, typed failures, and lossless or rejected-lossy save behavior pass. |
...
| GATE-47 | Roadmap gate | Open | The envelope-v2 model, migration, DOCX mapping, bounded reader, external-source lifecycle, fidelity state, and visible source replacement exist mechanically. Artifact `c3b2b54c` failed the packaged primary DOCX workflow despite correct build identity. A Word-authored fixture now passes the corrected production import/export path and LibreOffice comparison, but the corrected package has not received human validation. Parsed Markdown, ODT/RTF disposition, packaged save evidence, and complete format evidence remain open. | Document interoperability | Phase 47 | `RC-07` closes with the accepted format matrix, lifecycle, fidelity, lossiness, source-preservation, and fixture evidence. |
