...
| RC-07 | Release blocker | Open | Phase 47 mechanically implements bounded DOCX import, fidelity/lossiness state, provenance, fingerprint validation, source replacement, rollback, and path-free recovery. Artifact `8e974736` passed DOCX export. Artifact `2dfe312b` passed basic DOCX Open but failed source-format fidelity because accepted run, paragraph, and page-break formatting was flattened. Candidate `91fe1ba9`, built from correction commit `dc1d5d6b`, passes mechanical packaging checks but has no packaged-human or compatible-reader fidelity evidence. Parsed Markdown, governed ODT/RTF dispositions, and complete format evidence also remain absent. | Document interoperability | Phase 47 | Supported-format fixtures, Rust-owned lifecycle state, explicit fidelity classes, no-edit source preservation, typed failures, and lossless or rejected-lossy save behavior pass. |
...
| GATE-47 | Roadmap gate | Open | The envelope-v2 model, migration, DOCX mapping, bounded reader, external-source lifecycle, fidelity state, and visible source replacement exist mechanically. Packaged export and basic Open now pass separately, but artifact `2dfe312b` failed source-format fidelity. Candidate `91fe1ba9` contains the accepted run, paragraph, and page-break correction, but human fidelity evidence is absent. Parsed Markdown, ODT/RTF disposition, packaged save evidence, and complete format evidence remain open. | Document interoperability | Phase 47 | `RC-07` closes with the accepted format matrix, lifecycle, fidelity, lossiness, source-preservation, and fixture evidence. |
