...
| RC-07 | Release blocker | Open | Phase 47 mechanically implements bounded DOCX paragraph import, closed fidelity/lossiness categories, Rust-owned provenance, source-fingerprint validation, typed non-mutating eligibility, exact or consented-normalization replacement, atomic rollback, path-free recovery, and one visible confirmed Save Back workflow. Artifacts `910a204f` and `cb363385` failed human validation. Candidate `c3b2b54c` at commit `7ec149de` now has exact embedded build identity, visible DOCX dispositions, and `.draft` bundle registration, but its complete human retest is pending. Parsed Markdown, governed ODT/RTF dispositions, and complete format evidence remain absent. | Document interoperability | Phase 47 | Supported-format fixtures, Rust-owned lifecycle state, explicit fidelity classes, no-edit source preservation, typed failures, and lossless or rejected-lossy save behavior pass. |
...
| GATE-47 | Roadmap gate | Open | The envelope-v2 paragraph model, migration, editor preservation, DOCX property mapping, bounded DOCX reader, external-source lifecycle, fidelity/lossiness decisions, visible source replacement, and local reader evidence exist mechanically. Artifacts `910a204f` and `cb363385` failed their human gates. Candidate `c3b2b54c` mechanically passes embedded build identity, visible-outcome structure, and `.draft` association checks, but the exact packaged human gate remains pending. Parsed Markdown, ODT/RTF disposition, packaged save evidence, and complete compatible-reader evidence do not exist. | Document interoperability | Phase 47 | `RC-07` closes with the accepted format matrix, lifecycle, fidelity, lossiness, source-preservation, and fixture evidence. |
