...
| RC-08 | Release blocker | Open | Exact artifact `75373ffb` passed packaged review of the native File menu, compact controls, shared dispatch, busy and disabled states, Save As, bottom status placement, keyboard behavior, narrow layout, and purple identity, closing `UX-46-021` through `UX-46-023`. This is not final UI-design approval. `UX-46-024` remains governance-blocked under ADR-004, and final release disposition remains open. | Desktop UI and native workflow | Phase 48 | Native menus, visible controls, and shortcuts share one state-aware action path and pass packaged icon, layout, focus, enablement, and keyboard evidence; any expanded paragraph model first completes its separate governance path. |
...
| GATE-48 | Roadmap gate | Open | Exact artifact `75373ffb` passed the bounded Phase 48 packaged-human checklist and closed `UX-46-021` through `UX-46-023`. The result validates this implementation, not a final UI design; `RC-08` and governance-blocked `UX-46-024` remain open. | Desktop UI and native workflow | Phase 48 | `RC-08` closes with shared dispatch, native-menu parity, state, icon, layout, focus, and keyboard evidence. |
