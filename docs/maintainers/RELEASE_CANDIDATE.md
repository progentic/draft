...
| GATE-46 | Must close before Phase 49 | Open | Automated accessibility, keyboard, focus, announcement, state, recovery, responsive-layout, and visible-workflow checks pass; `UX-46-001` through `UX-46-003` are open, and the stable complete packaged workflow required to close `RC-01` through `RC-04` remains missing. | Frontend accessibility and workflow clarity | Phase 46 | Critical flows satisfy the accepted discoverability, understanding, state, recovery, keyboard, focus, naming, announcement, terminology, and unavailable-state criteria. |
